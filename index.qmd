...
time_data = {}
for name, label, hours in time_periods:
    time_data[name] = sum(hourly_data.get(h, 0) for h in hours)
max_time = max(time_data.values()) if time_data.values() and max(time_data.values()) > 0 else 1

# Determine peak coding time from the data
peak_time_name = max(time_data, key=time_data.get) if any(time_data.values()) else None
time_icons = {"Night": "🌙", "Morning": "☀️", "Afternoon": "🌤️", "Evening": "🌅"}
time_titles = {"Night": "Night Owl", "Morning": "Early Bird", "Afternoon": "Afternoon Warrior", "Evening": "Evening Coder"}
...
for name, label, hours in time_periods:
    value = time_data[name]
    heat = get_heat_level(value, max_time)
    is_peak = " (peak)" if name == peak_time_name and value > 0 else ""
    heatmap_html += f'''
        <div class="heatmap-row">
            <div class="heatmap-label">{name}</div>
            <div class="heatmap-cell heat-{heat}">{value}{is_peak}</div>
        </div>
    '''
...
# Add prominent coding time badge
if peak_time_name and time_data.get(peak_time_name, 0) > 0:
    icon = time_icons.get(peak_time_name, "💻")
    title = time_titles.get(peak_time_name, "Coder")
    heatmap_html += f'''
    <div class="coding-time-badge">
        <span class="coding-time-icon">{icon}</span>
        <span class="coding-time-title">You're a {title}</span>
    </div>
    '''
